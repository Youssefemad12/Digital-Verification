module axi4 #(
    ...
    // used the cnt instead of len because in the logic it gets decremented every beat (BUG FIX)
    ...
    localparam R_IDLE = 3'd0,
               R_ADDR = 3'd1,
               R_WAIT = 3'd2, // added wait state to fix the timing between the slave and the memory (BUG FIx)
               R_DATA = 3'd3;
    ...
    always @(posedge ACLK or negedge ARESETn) begin
        if (!ARESETn) begin
            ...
            case (read_state)
                ...
                R_WAIT : begin // wait for memory to read the correct address (BUG FIX)
                    read_state <= R_DATA;
                end
                R_DATA: begin
                    // Present read data
                    ...
                    if (RREADY && RVALID) begin
                        ...
                        if (read_burst_cnt > 0) begin
                            ...
                            read_state <= R_WAIT; // wait to capture the right addres (BUG FIX)
